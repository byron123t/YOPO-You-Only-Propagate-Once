...
def my_eval_one_epoch(net, batch_generator,  DEVICE=torch.device('cuda:0'), AttackMethod = None):
    ...
    if os.path.exists('labeled_indices'):
        os.remove('labeled_indices')
    if os.path.exists('natural_indices'):
        os.remove('natural_indices')
        
    dbfile = open('labeled_indices', 'ab')
    natfile = open('natural_indices', 'ab')
    pickle.dump(correct_indices, dbfile)
    pickle.dump(natural_indices, natfile)
    dbfile.close()
    natfile.close()
...
def process_single_epoch():
    ...
    filename = '../ckpts/binary-epoch181.checkpoint'
    print(filename)
